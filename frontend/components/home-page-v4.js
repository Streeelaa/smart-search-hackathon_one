...
function PersonalizedProducts({ items, onOpen }) {
  if (!items.length) {
    return null;
  }
  ...
  return (
    <section className="surface-block">
      <div className="section-topline">
        <span className="section-kicker">Популярно для заказчика</span>
      </div>
      <div className="personalized-product-list">
        {items.map((item) => (
          <button className="personalized-product-card" key={item.product.id} onClick={() => onOpen(item.product)} type="button">
            <img alt={item.product.title} className="personalized-thumb" src={getProductImage(item.product)} />
      ...
      <div className="card-layout">
        ...
        <div className="card-body">
          ...
          {attrs.length ? (
            ...
            <p className="muted-copy">Описание позиции появится здесь, когда по товару есть атрибуты.</p>
          )}
        </div>
      </div>
...
export function HomePageV4() {
    ...
    async function loadLanding() {
      try {
        ...
        const [overviewResponse, dashboardResponse] = await Promise.all([
          ...
          session?.user_id
            ? fetchJson(`/users/${session.user_id}/dashboard`, {
                searchParams: {
                  categories_limit: 8,
                  products_limit: 6,
                  favorites_limit: 4,
        ...
        if (!cancelled) {
          setError("Не удалось загрузить стартовые категории и персональные блоки.");
        }
      } finally {
        if (!cancelled) {
  ...
  function openPersonalizedProduct(product) {
    setQuery(product.title);
    setActiveCategory(product.category);
    void runSearch(product.title, product.category);
  }

  const landingCategories = dashboard?.preferred_categories?.length
    ? dashboard.preferred_categories
    : overview?.featured_categories || [];
      ...
      <section className="search-surface">
        ...
        {session?.organization_name ? (
          <div className="inline-row">
            <span className="badge-soft badge-accent">Профиль заказчика: {session.organization_name}</span>
          </div>
        ) : null}
          ...
          <section className="surface-block">
            <div className="section-topline">
              <span className="section-kicker">
                {dashboard?.preferred_categories?.length ? "Категории для этого заказчика" : "Популярные категории"}
              </span>
            </div>
            ...
            {loadingOverview ? <p className="muted-copy">Загружаем категории...</p> : null}
          </section>

          <PersonalizedProducts items={dashboard?.popular_products || []} onOpen={openPersonalizedProduct} />
        </>
      ) : (
        <section className="search-results-layout">
          <div className="results-main">
            <div className="results-inline-meta">
              ...
              {searchResponse.personalized && session?.organization_name ? <span>выдача адаптирована под профиль заказчика</span> : null}
            ...
            <div className="search-card-list">
              {searchResponse.items.map((item) => (
                <SearchCard
                  ...
                  onFavorite={() => void favoriteItem(item)}
